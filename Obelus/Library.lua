...
function library:makeResizable(frame) 
    ...
    local function handle_drag_input(input)
        return input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch
    end

    Frame.InputBegan:Connect(function(input)
        if handle_drag_input(input) then
            resizing = true
...
function library:initializeColorPicker(options) 
    ...
    local a = 0
        ...
        function cfg.updateColor() 
            ...
            local input_pos = vec2(mouse.X, mouse.Y - gui_offset)
            
            if draggingSaturation then	
                s = clamp((input_pos - val.AbsolutePosition).X / val.AbsoluteSize.X, 0, 1)
                v = 1 - clamp((input_pos - sat.AbsolutePosition).Y / sat.AbsoluteSize.Y, 0, 1)
            elseif draggingHue then 
                h = clamp(1 - (input_pos - hue.AbsolutePosition).Y / hue.AbsoluteSize.Y, 0, 1)
            elseif draggingAlpha then 
                a = clamp((input_pos - alpha.AbsolutePosition).Y / alpha.AbsoluteSize.Y, 0, 1)
            end
        ...
        local function handle_input_state(input)
            return input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch
        end

        alpha.InputBegan:Connect(function(input)
            if handle_input_state(input) then draggingAlpha = true end
        end)
        
        hue.InputBegan:Connect(function(input)
            if handle_input_state(input) then draggingHue = true end
        end)

        sat.InputBegan:Connect(function(input)
            if handle_input_state(input) then draggingSaturation = true end
        end)

        uis.InputEnded:Connect(function(input)
            if handle_input_state(input) then
...
function library:window(properties)
    local cfg = {
        ...
        size = properties.size or properties.Size or dim2(0, 516, 0.9, 0),
        selected_tab, 
...
function library:section(properties)
    ...
    function library:addSlider(options) 
        local cfg = {
            ...
            slider_dragger.InputBegan:Connect(function(input)
                if handle_input_state(input) then
                    cfg.dragging = true
                end
            end)
    ...
    function library:addDropdown(options) 
        ...
        flags[cfg.flag] = {} 
            ...
            dropdown:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
                dropdown_holder.Position = dim_offset(dropdown.AbsolutePosition.X, dropdown.AbsolutePosition.Y + dropdown.AbsoluteSize.Y + 1)
            end)
