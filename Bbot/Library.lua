...
-- Library functions 
    function Library:IsMobile()
        local platform = GuiService:GetPlatform()
        local is_phone = platform == Enum.Platform.Android or platform == Enum.Platform.IOS
        local small_screen = Workspace.CurrentCamera.ViewportSize.X < 800
        return is_phone or small_screen
    end
    ...
    function Library:CreateMobileToggle(window_instance)
        local Cfg = {
            Size = dim_offset(40, 40);
        }
        ...
        local Items = {}; do
            Items.Button = Library:Create("TextButton", {
                ...
                Position = dim_offset(Workspace.CurrentCamera.ViewportSize.X - 10, GuiService:GetGuiInset().Y + 10);
                Size = Cfg.Size;
                BackgroundTransparency = 1;
                Text = "";
                AutoButtonColor = false;
            ...
            }); Library:Themify(Items.Icon, "text_color", "ImageColor3")
        end

        Items.Button.MouseButton1Click:Connect(function()
            window_instance.ToggleMenu(not window_instance.Items.Window.Visible)
        end)
        
        if not Library:IsMobile() then
            Items.Button.Visible = false
        else
            -- Mobile: ensure button is visible if menu is closed by default
            Items.Button.Visible = not window_instance.Items.Window.Visible
        end
        
        window_instance.Items.Window:GetPropertyChangedSignal("Visible"):Connect(function()
    ...
    function Library:Window(properties)
        ...
        local is_mobile = Library:IsMobile()

        if is_mobile then
            Cfg.Size = dim2(0.95, 0, 0.9, 0)
        end
        ...
        local Items = Cfg.Items; do
            ...
            if is_mobile then
                Items.Window.AnchorPoint = vec2(0.5, 0.5)
                Items.Window.Position = dim2(0.5, 0, 0.5, 0)
            else
    ...
    function Library:Configs(window) 
        ...
        Section:Label({Name = "Background"}):Colorpicker({Flag = "Tab Background", Callback = function(color, alpha) 
...
local library_instance = Library:Window({Name = "Bbot v3"})

if library_instance:IsMobile() then
    library_instance:CreateMobileToggle(library_instance)
    library_instance.Items.Window.Visible = false
end

library_instance:Configs(library_instance)
