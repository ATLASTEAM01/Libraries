...
-- library init
	local library = {
		...
		-- Mobile specific
		mobile_drag = false, -- Default to false to prevent accidental drags
		drag_toggle_func = nil
	}
	...
	-- misc functions
		...
		function library:hovering(Object)
			if type(Object) == "table" then 
				...
				local y_cond = Object.AbsolutePosition.Y <= mouseLocation.Y and mouseLocation.Y <= Object.AbsolutePosition.Y + Object.AbsoluteSize.Y
				local x_cond = Object.AbsolutePosition.X <= mouseLocation.X and mouseLocation.X <= Object.AbsolutePosition.X + Object.AbsoluteSize.X
		...
		function library:make_resizable(frame) 
			...
			Frame.Position = dim2(1, -20, 1, -20) -- Bigger hit area for mobile
		...
		function library:draggify(frame)
			...
			frame.InputBegan:Connect(function(input)
				if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
					-- Mobile: Only allow drag if toggle is ON
					if not library.mobile_drag then return end
	...
	-- elements 
		...
		function library:panel(options) 
			local cfg = {
				...
				size = options.size or dim2(0, 500, 0, 400), -- Smaller default size for mobile
				position = options.position or dim2(0.5, 0, 0.5, 0), -- Center by default
				anchor_point = vec2(0.5, 0.5), -- Center anchor
		...
		function library:window(properties)
			...
			-- dock init
				dock_outline = library:create("Frame", {
					...
					Position = dim2(0.5, 0, 0, 10), -- Adjusted for mobile safe zone potentially
					Size = dim2(0, 185, 0, 39), -- Widen for extra button
					BorderSizePixel = 0,
					BackgroundColor3 = themes.preset.outline
				}); 
				...
				local UIListLayout = library:create("UIListLayout", {
					...
					HorizontalAlignment = Enum.HorizontalAlignment.Center, -- Center aligned
				...
				local DragToggleBtn = library:create("TextButton", {
					Parent = library.dock_holder,
					Name = "DragToggle",
					TextColor3 = rgb(0, 0, 0),
					BorderColor3 = rgb(0, 0, 0),
				...
				local drag_btn_inline = library:create("Frame", {
					...
					BorderSizePixel = 0,
					BackgroundColor3 = themes.preset.outline
				}) library:apply_theme(drag_btn_inline, "outline", "BackgroundColor3") 
				
				local drag_btn_inline2 = library:create("Frame", {
					Parent = drag_btn_inline,
					Name = "",
					Position = dim2(0, 1, 0, 1),
					BorderColor3 = rgb(0, 0, 0),
					Size = dim2(1, -2, 1, -2),
					BorderSizePixel = 0,
					BackgroundColor3 = rgb(255, 255, 255)
				}) library:apply_theme(drag_btn_inline2, "inline", "BackgroundColor3")
				
				local UIGradientDrag = library:create("UIGradient", {
					Parent = drag_btn_inline2,
					Name = "",
					Rotation = 90,
					Color = rgbseq{
						rgbkey(0, rgb(35, 35, 47)),
				...
				}) library:apply_theme(UIGradientDrag, "contrast", "Color") 
				
				local DragIcon = library:create("ImageLabel", {
					Parent = drag_btn_inline2,
					Name = "",
					ImageColor3 = themes.preset.inline, 
					Image = "rbxassetid://10723415903", -- Hand icon
					BackgroundTransparency = 1,
					BorderColor3 = rgb(0, 0, 0),
					Size = dim2(1, 0, 1, 0),
					BorderSizePixel = 0,
					BackgroundColor3 = rgb(255, 255, 255)
				}) library:apply_theme(DragIcon, "inline", "ImageColor3") 
				
				DragToggleBtn.MouseButton1Click:Connect(function()
					library.mobile_drag = not library.mobile_drag
					DragIcon.ImageColor3 = library.mobile_drag and themes.preset.accent or themes.preset.inline
				end)
			...
			-- main window
				local main_window = library:panel({
					...
					size = dim2(0, 500, 0, 400), -- Smaller default
